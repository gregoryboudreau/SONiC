...
- **Remote Access**: The authenticated gNOI Healthz `Artifact` RPC safely resolves and streams a completed DLDD artifact. It sends a size/SHA-256 header, bounded chunks, and a completion trailer; gNOI does not create or retain the archive. Legacy collection polling retries only the explicit not-ready state, returns permanent collection/check failures immediately, observes RPC cancellation, and resolves legacy artifact paths through the legacy-only containment boundary.
