...
Each action may specify `timeout` as a strict integer from 1 through 4294967295 seconds. If omitted, DLDD uses the top-level `local_action_default_timeout`. A sequence result is `COMPLETED`, `EXECUTION_ERROR`, or `TIMED_OUT`; command return values are vendor-owned and do not claim recovery success. An execution error or timeout stops later actions, but DLDD still triggers configured artifact collection and performs the configured wait and post-action recheck. That recheck alone determines whether the fault recovered.
...
Core DLDD checks that every requested reference is exposed, returned references match, direct results satisfy their contracts, and handle functions are callable. It does not invoke source expansion, enumerate instances, fetch a value, fetch a threshold, or execute an action/query during activation. An unresolved referenced hook rejects the complete rules file so operators cannot unknowingly activate a partially implemented contract.

The monitor invokes runtime handles only; typed direct results have already joined the ordinary adapter/evaluator path:

- `DSESourceHandle.expand(rule_context)` returns `DSEExpansionResult(bindings)` when discovery is due. Each successful result is the vendor API's current inventory for that selector. Each `DSEBinding` has a stable component `instance`, stable `source_id`, immutable vendor data, and optional value metadata.
...
- Common DLDD owns the adaptive refresh policy; vendors implement only discovery and access. The primary orchestration thread never calls DSE source/comparison runtime functions.
...
- Activation must reject the complete document if any signature or referenced installed hook is invalid. A valid DSE source template with zero instances before runtime expansion remains valid.
...
Validation is atomic for each candidate generation:

1. **Document gate**: Validates the root, exact registered `schema_version`, every nested signature, configured bounds, and cross-signature identity uniqueness. Any error rejects the complete candidate.
2. **Semantic conversion**: Converts the fully valid DTO document into immutable domain rules. Any context-free semantic error rejects the complete candidate.
3. **Shared preflight**: Checks platform compatibility, resolves all referenced installed DSE/action/query hooks, builds the work plan, and calls adapter configuration validation without sampling sources or executing operations. A missing hook or invalid static contract rejects the complete candidate.
4. **Runtime isolation**: After activation, expansion, get, comparison, collection, and action errors affect only their runtime rule/source and are reported through process telemetry. They never invalidate the file or trigger automatic rollback.

Normal activation does not call adapter source-read/collection methods, DSE expansion/source/comparison runtime functions, or local actions. Resolution and validation hooks must be side-effect-free. A DSE template is valid before any instance is discovered, and an instance that is absent from current successful expansion is simply not monitored. An unresolved referenced hook or invalid returned static contract rejects the candidate; later operation failures are localized runtime failures.
...
| Mode | Behavior |
|------|----------|
| `static-schema` | Bounded safe parsing, exact contract selection, complete strict Pydantic document validation, and context-free semantics. The historical name is retained, but no runtime JSON Schema is loaded. No platform extension discovery or source access occurs. |
...
3. **Atomic Document Validation**: Run strict version-specific Pydantic and context-free semantic validation across the complete document. Any error rejects the file while retaining bounded signature-path diagnostics.
...
7. **Activation Guard**: Activate only when the complete document and preflight pass; do not construct a partial plan.
...
1. **Build-time unit tests** are a mandatory build gate and cover the public contracts and high-risk paths in parsers, exact-version registries, Pydantic DTOs and validators, DTO-to-domain conversion, semantic validation, issue normalization, DSE reference parsing, typed contract validation, generated-schema tooling, and the two installed DLDD script entrypoints. Representative positive and primary failure behavior is required together with distinct wire-format and security boundaries; exhaustive permutations of equivalent fields, helpers, defaults, malformed values, or numeric edges are not required when they do not change the observable contract. Coverage-only calls are not a design requirement. Following the SONiC PMON convention, related normal, boundary, failure, and recovery states are grouped under the public behavior they exercise; independent wire-format and security matrices remain separately named only where that identity improves failure diagnosis. Common tests live directly under `sonic-host-services/tests/dldd/test_*.py` and run under the repository's normal CI policy. Platform-hook and SONiC utility code use their repository-local test suites without an additional DLDD-specific percentage gate.
2. **Runtime integration tests** execute whole-file activation/rejection, DSE expansion and adaptive refresh, runtime source/comparator invocation, localized operation failures, and representative fault/clear/action/recheck lifecycles with deterministic fake platform/Redis sources. The common threaded harness lives in `sonic-host-services/tests/dldd/integration/`.
...
Validation occurs at process activation after a candidate changes; runtime hot reload is not supported. A generation remains tied to the exact schema contract and checksum used to validate it. Any schema, semantic, or missing installed-hook failure rejects the complete candidate. After activation, operation failures are localized and never cause automatic selection of an older generation.
